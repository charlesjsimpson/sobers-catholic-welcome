...
export default function AdminUsers() {
  ...
  const createUser = async () => {
    ...
    // Sign up via supabase auth (admin invite would require service role, so we use signUp)
  ...
  const changeRole = async (profileId: string, newRole: AppRole) => {
    await supabase.from("user_roles").update({ role: newRole }).eq("user_id", profileId);
    await supabase.from("profiles").update({ role: newRole }).eq("id", profileId);
    fetch();
  };

  const roleLabels: Record<string, string> = {
    super_admin: "Super Admin", admin: "Admin", editor: "Éditeur", viewer: "Lecteur",
  };
